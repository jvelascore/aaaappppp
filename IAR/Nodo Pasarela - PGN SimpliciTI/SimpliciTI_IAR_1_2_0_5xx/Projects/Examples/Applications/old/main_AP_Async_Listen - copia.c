...
/* My appliction variables, constants and defs - Luis */

static uint_t typeOfDevice = 1; // 1- Access Point  0- Normal Node
  ...
  while (1)
  {
    ...
    if (sJoinSem && (sNumCurrentPeers < NUM_CONNECTIONS))
    {
      ...
      { /* SMPL_LinkListen will call nwk_PllBackgrounder for us if FHSS active */
        if (SMPL_SUCCESS == SMPL_LinkListen(&sLID[sNumCurrentPeers])) // if successful, this will cause a node to link
        {                                                             // we can access to its LinkID by accessing the array sLID
          break;
        }
...
{
  /* do something useful */
  /* send to Android device via UART*/
  if (len)
  {
    toggleLED(*msg);
